 ...
 *  ◤━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◥
 *      Core\Session
 *      จัดการ PHP Session อย่างปลอดภัย
 *      รองรับ prefix, timeout, flash, regenerate, destroy
 *
...
final class Session
{
    ...
    public static function start(int $timeout = 1800): void
    {
        ...
        try {
            ...
            // ตั้งค่าความปลอดภัย (cookie flags ผ่าน Cookie helper)
            ...
            session_set_cookie_params(Cookie::sessionParams(0));
    ...
    {
        $existing = self::get('flash_keys', []);
        $keys = is_array($existing) ? $existing : [];
        self::set('flash_' . $key, $value);
        self::set('flash_keys', array_merge($keys, [$key]));
    }
    ...
    public static function getFlash(string $key, mixed $default = null): mixed
    {
        ...
        $storedKeys = self::get('flash_keys', []);
        $keys = is_array($storedKeys) ? $storedKeys : [];
    ...
    public static function destroy(): void
    {
        if (self::$started && session_status() === PHP_SESSION_ACTIVE) {
            ...
            Cookie::forget(session_name(), [
                'path'     => $params['path'] ?? '/',
                'domain'   => $params['domain'] ?? '',
                'secure'   => (bool) ($params['secure'] ?? Cookie::secureDefault()),
                'httponly' => (bool) ($params['httponly'] ?? true),
                'samesite' => (string) ($params['samesite'] ?? Cookie::sameSiteDefault()),
            ]);
            session_destroy();
            self::$started = false;
        }
