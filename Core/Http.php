 ...
 *  ◤━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◥
 *      Core\Http
 *      HTTP client ด้วย cURL สำหรับเรียก API อื่น
 *      รองรับ GET POST PUT PATCH DELETE และ bearer token
 *
...
final class Http
{
    ...
    private static function request(string $method, string $url, mixed $data = null, array $headers = []): array
    {
        ...
        if (filter_var($url, FILTER_VALIDATE_URL) === false) {
            return self::error('URL ไม่ถูกต้อง');
        }

        $scheme = strtolower((string) parse_url($url, PHP_URL_SCHEME));
        if (!in_array($scheme, ['http', 'https'], true)) {
            return self::error('อนุญาตเฉพาะ http และ https');
        }
        ...
        if ($data !== null) {
            if (is_array($data) || is_object($data)) {
                ...
                $postData = is_string($data) ? $data : http_build_query((array) $data);
            }
